...
Save these three files, open `index.html` in the browser, type a birth year, and click the button.
...
Save, refresh, and try it again. The ReferenceError is gone and an age now appears. Try leaving the input blank and clicking the button again. Nothing crashes, but the result says you are 2026 years old, which is a logic error rather than a runtime error, since an empty string subtracted from a number just returns the number unchanged.
